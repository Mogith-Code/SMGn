...
<head>
<meta charset="utf-8">
<title>Official Grama Niladhari Certificate - ${cert.type}</title>
<style>
  body { font-family: 'Times New Roman', serif; margin: 0; padding: 40px; background-color: #f8fafc; }
  .certificate-border { border: 10px double #d4af37; padding: 40px; background-color: #ffffff; box-shadow: 0 4px 20px rgba(0,0,0,0.05); max-width: 800px; margin: auto; position: relative; }
  .state-emblem { text-align: center; margin-bottom: 20px; }
  .header-text { text-align: center; font-weight: bold; font-size: 18px; color: #1e293b; letter-spacing: 1px; margin-bottom: 5px; }
  .sub-header { text-align: center; font-size: 14px; color: #475569; font-style: italic; margin-bottom: 30px; }
  .cert-title { text-align: center; font-size: 26px; font-weight: 800; color: #1a2e56; text-transform: uppercase; margin-bottom: 40px; letter-spacing: 2px; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); }
  .content { font-size: 16.5px; line-height: 1.8; color: #334155; margin-bottom: 40px; text-align: justify; }
  .details-table { width: 100%; border-collapse: collapse; margin-bottom: 45px; }
  .details-table td { padding: 12px 10px; border-bottom: 1.5px solid #e2e8f0; }
  .details-table td.label { font-weight: bold; color: #475569; width: 35%; text-transform: uppercase; font-size: 12.5px; letter-spacing: 0.5px; }
  .details-table td.val { color: #1e293b; font-weight: 700; font-size: 15px; }
  .footer-row { display: flex; justify-content: space-between; align-items: flex-end; margin-top: 60px; }
  .stamp-box { border: 2.5px dashed #94a3b8; width: 120px; height: 120px; display: flex; align-items: center; justify-content: center; color: #94a3b8; font-size: 11.5px; font-weight: bold; border-radius: 50%; text-transform: uppercase; text-align: center; line-height: 1.4; }
  .signature-box { text-align: center; width: 220px; }
  .signature-line { border-top: 1.5px solid #1e293b; margin-top: 50px; font-weight: 750; color: #1e293b; font-size: 13.5px; text-transform: uppercase; letter-spacing: 0.5px; }
  @media print {
    body { background-color: #ffffff; padding: 0; }
    .certificate-border { box-shadow: none; border-color: #d4af37 !important; }
  }
</style>
</head>
<body>
<div class="certificate-border">
  <div class="state-emblem">
    <svg width="80" height="80" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="32" cy="32" r="28" stroke="#d4af37" stroke-width="2.5" fill="none" />
      <circle cx="32" cy="32" r="24" stroke="#d4af37" stroke-width="1.5" fill="none" />
      <polygon points="32,15 36,26 46,26 38,32 41,42 32,36 23,42 26,32 18,26 28,26" fill="#d4af37" />
    </svg>
  </div>
  <div class="header-text">DEMOCRATIC SOCIALIST REPUBLIC OF SRI LANKA</div>
  <div class="sub-header">Office of the Grama Niladhari Secretariat</div>
  
  <div class="cert-title">${cert.type}</div>
  
  <div class="content">
    This is to officially certify and verify that the resident credentials listed below have been vetted, audited, and authenticated against the voter registrations, household logs, and utility records of the Borella Gramaseva Division. Accordingly, this document is officially issued for the designated administrative purposes.
  </div>
  
  <table class="details-table">
    <tr>
      <td class="label">Certificate Type</td>
      <td class="val">${cert.type}</td>
    </tr>
    <tr>
      <td class="label">Applicant Name</td>
      <td class="val">${successUser}</td>
    </tr>
    <tr>
      <td class="label">Designated Purpose</td>
      <td class="val">${cert.purpose}</td>
    </tr>
    <tr>
      <td class="label">Gramaseva Division</td>
      <td class="val">Colombo, Borella</td>
    </tr>
    <tr>
      <td class="label">Issue Date & Time</td>
      <td class="val">${cert.approvedDate}</td>
    </tr>
  </table>
  
  <div class="footer-row">
    <div class="stamp-box">
      Official<br>Divisional<br>Seal
    </div>
    <div class="signature-box">
      <svg width="150" height="40" viewBox="0 0 150 40">
        <path d="M15,25 Q35,5 65,35 T115,15 T135,25" stroke="#1d4ed8" stroke-width="2.5" fill="none"/>
      </svg>
      <div class="signature-line">Grama Niladhari Signature</div>
    </div>
  </div>
</div>
<script>
  window.onload = function() {
    ...
    // Create a Blob from the HTML content and trigger browser download
    const blob = new Blob([htmlContent], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Certificate_${cert.type.replace(/\s+/g, '_')}_${cert.id}.html`;
